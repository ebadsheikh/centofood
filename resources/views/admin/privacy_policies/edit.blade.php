                ...
                <div id="kt_app_toolbar_container" class="app-container d-flex flex-stack">
                    <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                        ...
                        <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                            ...
                            <li class="breadcrumb-item text-muted">{{ __('edit_privacy_policy') }}</li>
                        </ul>
                    </div>
                </div>
            </div>
            <div id="kt_app_content" class="app-content flex-column-fluid">
                <div id="kt_app_content_container" class="app-container">
                    ...
                    <form class="form d-flex flex-column flex-lg-row" method="POST"
                        action="{{ route('privacy.policy.update', $privacyPolicy->slug) }}">
                        @csrf
                                ...
                                <div class="card-body pt-0">
                                    <div class="col-lg-12 fv-row fv-plugins-icon-container">
                                        ...
                                        <input type="text" name="slug" class="form-control"
                                            placeholder="{{ __('enter_slug') }}" value="{{ $privacyPolicy['slug'] }}">
                                        @if ($errors->has('slug'))
                                    ...
                                    @foreach (config('translatable.locales') as $translation)
                                        <div class="col-lg-12 fv-row fv-plugins-icon-container">
                                            <label class="col-lg-12 col-form-label fw-bold fs-6">{{ __('privacy_policy') }}
                                                ({{ $translation }})</label>
                                            <textarea name="{{ $translation }}[description]" class="kt_docs_ckeditor_classic">{{ old($translation . '.description', $privacyPolicy->hasTranslation($translation) ? $privacyPolicy->translate($translation)->description : '') }}</textarea>
