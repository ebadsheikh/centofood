        ...
        <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
                ...
                <input type="text" wire:model="searchName" class="form-control form-control-solid w-250px ps-15"
                    placeholder="{{ __('search_privacy_policy') }}" />
            </div>
        </div>
            ...
            <div class="card-toolbar">
                <div class="d-flex justify-content-end">
                    <a href="{{ route('privacy.policy.create') }}" class="btn btn-primary">
                        ...
                        </span>{{ __('add_new_privacy_policy') }}</a>
                </div>
            </div>
        @endcan --}}
    </div>
    <div class="card-body pt-0">
        <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_datatable_example">
            ...
            <tbody class="fw-semibold text-gray-600">
                @forelse ($privacyPolicies as $key => $policy)
                    <tr class="text-center">
                        <td class="text-dark">{{ $key + 1 }}</td>
                        <td class="text-dark">{{ $policy->slug }}</td>
                        <td class="text-dark">{{ strip_tags($policy->description) }}</td>
                        <td class="text-dark">
                            @can('privacy.policy.edit')
                                <a href="{{ route('privacy.policy.edit', $policy->slug) }}"
                                    class="btn btn-icon btn-success btn-sm mr-2">
                                    <i class="bi bi-pencil fs-4"></i>
                                </a>
                            @endcan
                            {{-- @can('privacy.policy.delete')
                                <button data-bs-toggle="modal" data-bs-target="#kt_modal_{{ $policy->slug }}"
                                    class="btn btn-icon btn-danger btn-sm mr-2">
                                    <i class="bi bi-trash fs-4"></i>
                                </button>
                            @endcan
                            <div class="modal fade" tabindex="-1" id="kt_modal_{{ $policy->slug }}">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer d-flex justify-content-center">
                                            <button type="button" class="btn btn-light"
                                                ...
                                                {{ __('cancel') }}</button>
                                            <a href="{{ route('privacy.policy.delete', $policy->slug) }}" type="button"
